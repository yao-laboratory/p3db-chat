    ...
    for filename in os.listdir(folder_path):
        if filename.endswith((".png", ".jpg", ".jpeg")):
            ...
            if messageParsed:  # Only create the DataFrame if there is something to parse
                ...
                with pd.ExcelWriter('repeat_10/10_'+filename.split('.')[0] + '_results.xlsx', engine='openpyxl', mode='a') as writer:
                    # Write the main DataFrame to the first sheet
                    df.to_excel(writer, sheet_name='onestep', index=False)
                # df.to_csv(image_path + '_results.csv', index=False)
